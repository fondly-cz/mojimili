...
5. **Deploy the stack.** Portainer builds the image, starts MariaDB, waits for it, then runs
   `migrate --force`, seeds the service catalog (first deploy only), creates the admin, runs
   `optimize` and starts the app. **After the first deploy**, log in as admin and generate the
   OAuth keys under **Nastavení → MCP / API klíče** — until then the MCP server returns an error.
...
> data lives in two volumes: `mojimili-db` (database) and `mojimili-storage` (uploads and logs). The
> Passport OAuth keys live in the database (`passport_keys`, generated from the admin), so they
> survive a redeploy or a wiped `mojimili-storage` volume on their own.
...
The OAuth keys are generated by an admin under **Nastavení → MCP / API klíče** and stored in the
database (`passport_keys` table, private key encrypted with `APP_KEY`), so they survive redeploys and
volume wipes. Do this once after the first deploy — until the keys exist the MCP server returns an
error. `APP_URL` must be the production HTTPS domain (`compose.prod.yml` derives it from
`DOMAIN_NAME`), as it doubles as the OAuth issuer.
