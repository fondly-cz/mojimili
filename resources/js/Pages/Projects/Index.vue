<template>
    <Layout>
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <h1 class="text-4xl font-extrabold text-gray-900 font-heading tracking-tight">Projekty</h1>
                <p class="text-gray-500 mt-2 font-medium">Sledujte rozpracované zakázky a jejich seznamy úkolů.</p>
            </div>
            <div>
                <Link
                ...
                <div class="md:col-span-6">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            type="text"
                            id="search"
                            placeholder="Hledat podle názvu projektu nebo firmy..."
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="border-b border-gray-50">
                            ...
                            <th class="px-8 py-6 text-left text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 font-heading">Projekt</th>
                            <th class="px-8 py-6 text-left text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 font-heading">Firma</th>
                            <th class="px-8 py-6 text-left text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 font-heading">Seznamy</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        <tr v-if="projects.data.length === 0">
                            <td colspan="6" class="px-8 py-20 text-center">
                                <p class="text-sm font-bold text-gray-400">Zatím tu není žádný projekt.</p>
                                <Link href="/projects/create" class="mt-3 inline-block text-sm font-black brand-text-gradient">Vytvořit první projekt</Link>
                            </td>
                        </tr>
                        <tr v-for="project in projects.data" :key="project.id" :class="{'bg-brand-primary-from/5': selectedIds.includes(project.id)}" class="hover:bg-gray-50/50 transition-colors group">
                            <td class="px-8 py-6">
                                <input
                                    type="checkbox"
                                    v-model="selectedIds"
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div class="text-base font-bold text-gray-900 font-heading leading-tight">{{ project.name }}</div>
                                        <div v-if="project.description" class="text-xs font-semibold text-gray-400 mt-1 line-clamp-1 max-w-md">
                                            {{ project.description }}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="px-8 py-6">
                                <Link v-if="project.company" :href="`/companies/${project.company.id}`" class="text-sm font-bold text-gray-900 hover:text-brand-primary-from transition-colors">
        ...
        <!-- Pagination -->
        <div v-if="projects.links.length > 3" class="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <p class="text-sm font-bold text-gray-400">
                    Zobrazeno {{ projects.from }} až {{ projects.to }} z {{ projects.total }} výsledků
                </p>
                <select
...
const search = () => {
    router.get('/projects', searchForm, {
        preserveState: true,
        replace: true,
    })
}
...
const deleteProject = (project) => {
    confirmDelete.title = 'Smazat projekt'
    confirmDelete.message = `Opravdu chcete smazat projekt "${project.name}"? Smažou se i jeho seznamy úkolů.`
...
const toggleSelectAll = (e) => {
    selectedIds.value = e.target.checked ? props.projects.data.map(p => p.id) : []
}

const bulkDelete = () => {
    confirmDelete.title = 'Hromadné smazání'
    confirmDelete.message = `Opravdu chcete smazat ${selectedIds.value.length} vybraných projektů včetně jejich seznamů úkolů?`
...
const executeDelete = () => {
    if (confirmDelete.type === 'single') {
        router.delete(`/projects/${confirmDelete.item.id}`, {
